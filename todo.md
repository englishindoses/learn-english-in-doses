...
- [ ] **`CLAUDE.md`** — on hold until the skill settles, so it can describe the real workflow rather than a version that is about to change. Short when written: what the site is, folder map, hard rules (British spelling, no build step, no frameworks, edit HTML directly), and pointers to the guideline docs rather than copies of them.
